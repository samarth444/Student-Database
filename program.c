...
void AddNewStudent()
{
    ...
    while(!IsValidID)
    {
        ...
        {
            printf(" Error: This SRN is already exists.\n\n");
            IsValidID = 0;
        }
        else if(strlen(StudentID) > 10)
        {
            printf(" Error: SRN can not be more than 10 characters.\n\n");
            IsValidID = 0;
        }
        else if(strlen(StudentID) <= 0)
        {
            printf(" Error: SRN can not be empty.\n\n");
            IsValidID = 0;
        }
        else
        {
    ...
    strcpy(Students[TotalStudents].SRN,StudentID);
...
{
    printf("|==========|====================|==============================|====================|=============|\n");
    printf("|    SRN   |        Name        |            Email             |       Phone        |  NO.Course  |\n");
    printf("|==========|====================|==============================|====================|=============|\n");

    for(i=0; i<TotalStudents; i++)
    {
        printf("|");
        printf("%s",Students[i].SRN);
        for(j=0; j < (10-strlen(Students[i].SRN)); j++)
        {
            printf(" ");
        }
        printf("|");
...
int SearchStudent(char StudentID[10])
{
    ...
    for(i=0; i<TotalStudents; i++)
    {
        if(strcmp(StudentID,Students[i].SRN) == 0)
        {
            StudentFoundIndex = i;
            printf("\n One Student Found for SRN: %s\n\n",StudentID);
...
void EditStudent(int StudentFoundIndex)
{
    ...
    char StudentID[300];
    strcpy(StudentID, Students[StudentFoundIndex].SRN);
...
void DeleteStudent(int StudentIndex)
{
    ...
    {
        if(strcmp(ThisStudents.SRN,Courses[d].StudentID) == 0)
        {
            FirstCourseIndexs = d;
            break;
        }
...
int IsAlreadyExists(char GivenLine[300],char InfoType, char StudentID[300])
{
    ...
    {
        if(strcmp(GivenLine,Students[ep].SRN) == 0)
        {
            IDExists++;
        }
        if(strcmp(GivenLine,Students[ep].Email) == 0 && strcmp(StudentID,Students[ep].SRN) != 0 )
        {
            EmailExists++;
        }
        if(strcmp(GivenLine,Students[ep].Phone) == 0 && strcmp(StudentID,Students[ep].SRN) != 0)
        {
            PhoneExists++;
        }
...
{
    printf("\n\t\t **** How it Works? ****\n\n");
    printf(" -> You will only able to store the Student's SRN, Name, Email, Phone, Number of Courses.\n");
    printf(" -> A student can have maximum 4 courses and minimum 1 course.\n");
    printf(" -> Student SRN can be maximum 10 characters long and unique for every students.\n");
...
void AboutUs()
{
    ...
    printf("\n\t\t **** V Rishi    PES1UG22CS475 ****\n\n");
    printf("\n\t\t **** Ruthvin V  PES1UG22CS489 ****\n\n");
...
void DataSeed()
{
    ...
    //-- student 1
    strcpy(Students[0].SRN,"S-1");
    strcpy(Students[0].Name,"Student 1");
    strcpy(Students[0].Phone,"1478523690");
    ...
    //-- student 2
    strcpy(Students[1].SRN,"S-2");
    strcpy(Students[1].Name,"Student 2");
    strcpy(Students[1].Phone,"9876543210");
    ...
    //-- student 3
    strcpy(Students[2].SRN,"S-3");
    strcpy(Students[2].Name,"Student 3");
    strcpy(Students[2].Phone,"1234567891");
